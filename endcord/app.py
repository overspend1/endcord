...
cythonized = importlib.util.find_spec("endcord_cython") and importlib.util.find_spec(
...
APP_COMMAND_AUTOCOMPLETE_DELAY = (
    0.3  # delay for requesting app command autocompletions after stop typing
)
MB = 1024 * 1024
USER_UPLOAD_LIMITS = (
    10 * MB,
...
class Endcord:
    ...
    def __init__(
        self, screen, config, keybindings, command_bindings, profiles, version
    ):
        self.screen = screen
        self.config = config
        ...
        self.chat = []
        self.chat.insert(
            0, _("connecting", host=self.config["custom_host"] or "Discord")
        )
        logger.info(_("connecting", host=self.config["custom_host"] or "Discord"))
    ...
    def load_extensions(self, version):
        ...
        for ext_file, ext_name in extensions:
            ...
            sys.path.insert(
                0, ext_dir
            )  # add extension dir to sys.path to load their modules
            try:
                ...
                if not callable(ext_class):
                    log_text_invalid.append(
                        f"  {ext_name} - ERROR: Extension class is invalid"
                    )
                    continue
                ...
                if ext_app_version.split(".")[:2] != version.split(".")[:2]:
                    log_text.append(
                        f"  {ext_name} {ext_version} - WARNING: This extension is built for different endcord version!"
                    )
                else:
        ...
        for ext_file, ext_name in invalid:
            log_text_invalid.append(
                f"  {ext_name} - ERROR: Invalid extension structure"
            )
        if log_text:
            logger.info(
                f"Loaded {len(self.extensions)} extensions:\n" + "\n".join(log_text)
            )
        if log_text_invalid:
        ...
        self.chat.insert(
            0,
            f"Not loaded (invalid) {len(extensions) - len(self.extensions) + len(invalid)} extensions",
        )
        self.tui.update_chat(self.chat, [[[self.colors[0]]]] * len(self.chat))
    ...
    def wait_slowmode(self):
        ...
        while self.slowmodes and self.slowmode_times:
            ...
            if not all_times:
                break
            status = (
                "%slowmode" in self.format_status_line_r
                or "%slowmode" in self.format_status_line_l
            )
            title = (
                "%slowmode" in self.format_title_line_r
                or "%slowmode" in self.format_title_line_l
            )
    ...
    def reconnect(self):
        ...
        self.add_running_task(_("reconnecting"), 1)
    ...
    def switch_channel(
        ...
        open_member_list = (
            ...
            and guild_id != self.active_channel["guild_id"]
        ...
        if (
            self.active_channel["channel_id"]
            and self.active_channel["channel_id"] in self.read_state
        ):
            channel = self.read_state.get(channel_id)
            if (
                not channel
                or channel["last_message_id"] == channel["last_acked_message_id"]
            ):
                self.read_state[self.active_channel["channel_id"]][
                    "last_acked_unreads_line"
                ] = None
        ...
        if not forum:
            ...
            if (
                ...
                and int(channel["last_acked_message_id"])
            ...
            if last_acked_msg:
                for num, message in enumerate(self.messages):
                    if int(message["id"]) <= last_acked_msg:
                        select_message_index = min(
                            max(num - 1, 0), len(self.messages) - 2
                        )
                        break
                else:
            ...
            if channel_id in self.incoming_calls:
                ...
                for dm in self.dms:
                    if dm["id"] == incoming_call_ch_id:
                        ...
                        break
                if (
                    new_permanent_extra_line
                    and new_permanent_extra_line != self.permanent_extra_line
                ):
                    self.update_extra_line(
                        custom_text=new_permanent_extra_line, permanent=True
                    )
        ...
        if guild_id:
            if self.get_members:
                for guild in self.members:
                    if guild[0] == guild_id:
                        if channel_id in guild[1]:
                            self.member_list = guild[1][channel_id][1]
                        elif "everyone" in guild[1]:
                            self.member_list = guild[1]["everyone"][1]
                        elif guild[1]:
                            self.member_list = next(iter(guild[1].values()))[1]
                        else:
                            self.member_list = []
                        break
                else:
        ...
        if not forum:
            self.update_chat(
                keep_selected=False,
                select_message_index=select_message_index,
                select_unread=True,
            )
        else:
            self.tui.update_chat(self.chat, self.chat_format)
        self.set_channel_seen(
            channel_id, self.get_chat_last_message_id(), force_remove_notify=True
        )  # right after update_chat so new_unreads is determined
        if not guild_id:  # no member list in dms
            ...
            if delay:
                time.sleep(
                    0.01
                )  # needed when startup to fix issues with emojis and border lines
        ...
        if one_open:
            ...
            for obj in self.tree_metadata:
                if obj and obj["type"] < 0:  # guilds and folders
                    ...
                    if (
                        direction == 0
                        or (direction == -1 and not passed)
                        or (direction == 1 and passed)
                    ):
    ...
    def select_current_channels(self, parent_hint=None, refresh=False):
        ...
        if (
            slowmode
            and not this_guild.get("admin", False)
            and not self.current_channel.get("bypass_slowmode")
        ):
    ...
    def add_to_channel_cache(self, channel_id, messages, set_pinned):
        ...
        if self.limit_channel_cache:
            ...
            if (
                self.get_chat_last_message_id() == self.last_message_id
    ...
    def toggle_tab(self):
        ...
        if not self.forum:
            ...
            if self.config["remember_tabs"]:
                tabbed_channels = [
                    channel[0] for channel in self.channel_cache if channel[2]
                ]
                ...
                peripherals.save_json(
                    self.state, f"state_{self.profiles['selected']}.json"
                )
    ...
    def wait_input(self, forced_binding=None):
        ...
        while self.run and not ephemeral:
            ...
            if action == 4:
                ...
                if tree_sel is None or tree_sel < 0:
                    continue
            ...
            # edit
                ...
                message = self.messages[msg_index]
                if (
                    message["user_id"] == self.my_id
                    and "deleted" not in message
            ...
            # delete
                ...
                message = self.messages[msg_index]
                if (
                    (message["user_id"] == self.my_id or self.active_channel["admin"])
                    and "deleted" not in message
                    and "pending" not in message
                ):
                    self.reset_states()
            ...
            # view media
                ...
                urls = self.get_msg_urls_chat(msg_index)
                if urls:
                    for num in self.get_stuff_from_selected_line(
                        chat_sel, 0
                    ):  # get url
                        if urls[num] in embeds:
                            selected_urls.append(urls[num])
            ...
            # moving left/right through attachments
                ...
                self.restore_input_text = (input_text, "standard")
                num_attachments = 0
                attachments = self.ready_attachments.get(
                    self.active_channel["channel_id"]
                )
                if attachments:
            ...
            # select in extra window / member list
                ...
                if self.extra_window_open and action == 27:
                    ...
                    self.state.get("member_list") and self.active_channel["guild_id"]
                ):  # controls for member list when no extra window
            ...
            # open external editor
                ...
                temp_message_path = os.path.join(
                    peripherals.temp_path, f"message-{timestamp}"
                )
            ...
            # up/down in command mode
                ...
                if self.command_history and self.command_history_index <= history_len:
                    if self.command_history_index < history_len:
                        ...
                        self.restore_input_text = (
                            self.command_history_stored_current,
                            "command",
                        )
                        self.command_history_stored_current = None
                        self.command_history_index += 1
                ...
                if clicked_chat is not None and mouse_x is not None:
                    ...
                    if self.forum:
                        ...
                        if (
                            ...
                            clicked_type == 4 and clicked_id is not None
                        ):  # add/remove reaction
                            self.build_reaction(
                                str(clicked_id + 1), msg_index=msg_index
                            )
                        elif clicked_type == 5:  # url
                            urls = self.get_msg_urls_chat(msg_index)
                            content_urls = []
                            for match in re.finditer(
                                formatter.match_url, self.messages[msg_index]["content"]
                            ):
                                content_urls.append(match.group())
                            url = urls[clicked_id]
                            embed_url = False
                            for embed in self.get_msg_embeds(
                                msg_index, media_only=False, stickers=False
                            ):
                            ...
                            if match:
                                if (
                                    support_media
                                    and shutil.which(self.config["yt_dlp_path"])
                                    and shutil.which(self.config["mpv_path"])
                                ):
                                    self.download_threads.append(
                                        threading.Thread(
                                            target=self.download_file,
                            ...
                            (
                                ...
                                guild_name,
                                parent_hint,
                            ) = self.find_parents_from_id(clicked_id)
                            if channel_name:
                                self.switch_channel(
                                    channel_id,
                                    channel_name,
            ...
            # mouse single-click on extra line
                ...
                if (
                    ...
                    and not self.extra_window_open
                ):
                    if self.in_call:
                        mouse_x = self.tui.get_extra_line_clicked()
                        muted = bool(self.state.get("muted")) * 2
                        if (
                            len(self.extra_line) - 13 - muted
                            <= mouse_x
                            < len(self.extra_line) - 9
                        ):  # TOGGLE MUTE
                            if muted:
                            ...
                            len(self.extra_line) - 6
                            <= mouse_x
                            < len(self.extra_line) - 1
                        ):  # LEAVE
                            self.leave_call()
                    elif (
                        self.most_recent_incoming_call
                        or self.active_channel["channel_id"] in self.incoming_calls
                    ):
                        mouse_x = self.tui.get_extra_line_clicked()
                        if (
                            len(self.extra_line) - 16
                            <= mouse_x
                            < len(self.extra_line) - 10
                        ):  # ACCEPT
                            if not self.in_call:
                                if self.most_recent_incoming_call:
                                    incoming_call_ch_id = self.most_recent_incoming_call
                                else:
                                    incoming_call_ch_id = self.active_channel[
                                        "channel_id"
                                    ]
                                threading.Thread(
                                    target=self.start_call,
                                    daemon=True,
                                    args=(True, None, incoming_call_ch_id),
                                ).start()
                            else:
                                self.update_extra_line("Cant join multiple calls")
                        elif (
                            len(self.extra_line) - 7
                            <= mouse_x
                            < len(self.extra_line) - 1
                        ):  # REJECT
                            self.stop_ringing()
                            ...
                            if (
                                self.active_channel["channel_id"]
                                not in self.incoming_calls
                            ):
                                self.update_extra_line(permanent=True)
                elif (
                    self.fun == 4
                    and self.extra_line
                    and self.extra_line.startswith("Personalized")
                ):
                    self.download_threads.append(
                        threading.Thread(
                            target=self.download_file,
                    ...
                    for command in parser.split_command_binding(action[1]):
                        ...
                        if not binding:
                            self.execute_command(
                                cmd_type, cmd_args, action[1], chat_sel, tree_sel
                            )
            ...
            elif self.execute_extensions_method_first(
                "on_wait_input", action, input_text, chat_sel, tree_sel, cache=True
            ):
                pass
            ...
            # enter
                ...
                if self.editing:
                    ...
                    self.close_extra_window()
                    self.execute_command(
                        command_type, command_args, input_text, chat_sel, tree_sel
                    )
                    self.assist_word = None
                    ...
                    self.restore_input_text = (input_text, "standard")
                    self.update_extra_line(
                        f"Slowmode is enabled, will be able to send message in {self.slowmode_times[self.active_channel['channel_id']]} s"
                    )
                    ...
                    if attachments:
                        if all(
                            attachment.get("state", 0) >= 1
                            for attachment in attachments
                        ):
                            this_attachments = self.ready_attachments.pop(
                                active_channel
                            )
                            self.update_extra_line()
                        else:
                    ...
                    if self.current_channel.get("type") in (
                        ...
                        channel_id, _, guild_id, _, parent_id = (
                            self.find_parents_from_tree(active_channel)
                        )
    ...
    def execute_command(self, cmd_type, cmd_args, cmd_text, chat_sel, tree_sel):
        ...
        if cmd_type == 0:
            if cmd_args:
                ...
                "on_execute_command", cmd_text, chat_sel, tree_sel
            ):
                pass
            else:
        ...
        if cmd_type == 1:  # SET
            ...
            if urls:
                ...
                if len(selected_urls) == 1 or select_num:
                    select_num = max(min(select_num - 1, len(selected_urls) - 1), 0)
                    selected_url = self.refresh_attachment_url(
            ...
            if len(selected_urls) == 1 or select_num:
                ...
                self.download_threads.append(
                    threading.Thread(
                        target=self.download_file,
                        daemon=True,
                        args=(selected_urls[select_num], False, True),
                    )
                )
                self.download_threads[-1].start()
            else:
            ...
            if not react_text:
                reset = False
                if (
                    "deleted" not in self.messages[msg_index]
                    and "pending" not in self.messages[msg_index]
                ):
                    self.restore_input_text = (None, "react")
                    self.ignore_typing = True
                    self.reacting = {
            ...
            # channel/dm
                ...
                if tp:
                    self.update_extra_line(
                        "You're inside building. There is food here."
                    )
                ...
                for num, channel in enumerate(self.input_store):
                    if channel["id"] == active_channel:
                        ...
                        self.input_store[num]["content"] = (
                            input_text[:input_index] + paths + input_text[input_index:]
                        )
                ...
                if poll["expires"] > time.time():
                    if select_num > 0 and select_num <= len(poll["options"]):
                        ...
                        if poll["options"][select_num]["me_voted"]:
                            # clear voted
                            success = self.discord.send_vote(
                                self.active_channel["channel_id"],
            ...
            if (
                ...
                if not self.active_channel["guild_id"] or (
                    ...
                    or self.active_channel.get("allow_manage")
                ):
            ...
            if "text" in cmd_args:
                ...
                if match:
                    ...
                    self.my_status["custom_status_emoji"] = {
                        "id": None,
                        "name": emoji.emojize(
                            cmd_args["emoji"], language="alias", variant="emoji_type"
                        ),
                        "animated": False,
                    }
            else:
            ...
            if self.my_status["custom_status"]:
                settings["status"]["customStatus"]["text"] = self.my_status[
                    "custom_status"
                ]
            if self.my_status["custom_status_emoji"]:
                settings["status"]["customStatus"]["emojiName"] = self.my_status[
                    "custom_status_emoji"
                ]["name"]
            if not self.gateway.legacy:
                success = self.discord.patch_settings_proto(1, settings)
            else:
                success = self.discord.patch_settings_old(
                    "custom_status", {"text": self.my_status["custom_status"]}
                )
            if success:
                self.gateway.update_presence(
                    self.my_status["status"],
            ...
            if not self.in_call:
                if not self.active_channel["guild_id"]:
                    threading.Thread(
                        target=self.start_call,
                        daemon=True,
                        args=(False, None, self.active_channel["channel_id"]),
                    ).start()
                else:
            ...
            if self.active_channel["guild_id"]:
                ...
                if invite_url:
                    ...
                    self.update_extra_line(
                        "Failed to generate invite, see log for more info"
                    )
            ...
            if cmd_args.get("name"):
                ...
                if match:
                    ...
                    self.update_extra_line(
                        "Invalid emoji. Should be: <:EmojiName:emoji_id>"
                    )
            else:
                select_num = cmd_args.get("num", 0)
                if not select_num:
            ...
            msg_index = (
                self.lines_to_msg(self.tui.get_chat_selected()[0], space=True) + 1
            )
            msg_index = min(msg_index, len(self.messages) - 1)
            message_id = self.messages[msg_index]["id"]
            if message_id:
                self.send_ack(
                    self.active_channel["channel_id"], message_id, manual=True
                )
        ...
        elif cmd_type == 60 and self.current_channel.get("type") in (
            11,
            12,
        ):  # TOGGLE_THREAD
            ...
            if self.enable_game_detection and self.game_detection.run:
                ...
                self.update_extra_line(
                    "Game detection service is disabled or not running."
                )
            ...
            threading.Thread(
                target=self.send_message_command,
                daemon=True,
                args=(
                    cmd_args["content"],
                    cmd_args["channel_id"],
        ...
        # 64 - COPY_LINK is handled together with 5 - OPEN_LINK
            ...
            if target_id is not None:
                self.execute_command(
                    25, {"channel_id": target_id}, "", chat_sel, tree_sel
                )  # goto command
                return

        elif cmd_type == 71:  # VOICE_JOIN_CALL
            channel_id = cmd_args.get("channel_id")
            if not channel_id:
                if (
                    self.tree_metadata[tree_sel]
                    and self.tree_metadata[tree_sel]["type"] == 2
                ):
                    channel_id = self.tree_metadata[tree_sel]["id"]
            if channel_id:
                _, _, guild_id, _, _ = self.find_parents_from_id(channel_id)
                if not self.in_call:
                    threading.Thread(
                        target=self.start_call,
                        daemon=True,
                        args=(False, guild_id, channel_id),
                    ).start()
                else:
                    self.update_extra_line("Cant join multiple calls")

        elif cmd_type == 72:  # VOICE_MUTE
            self.state["muted"] = True
            if self.in_call:
                self.update_voice_mute_in_call()
            else:
    ...
    def send_message_command(
        self, content, channel_id, reply_id, ping, attachments_paths
    ):
            ...
            if embed["url"] and (not media_only or media_type in MEDIA_EMBEDS):
                if media_type == "rich":
                    for num, match in enumerate(
                        re.finditer(formatter.match_url, embed["url"])
                    ):
                        if (
                            "media" in embed
                            and len(embed["media"]) >= num
                            and embed["media"][num]
                        ):
                            urls.append(match.group())
    ...
    def refresh_attachment_url(self, url):
        """Try to refresh discord attachment url if it is expired"""
        if any(x in url for x in discord.DISCORDAPP_CDN_ATTACHMENTS):
            if "ex=" in url:
                # check if expired
                try:
                    ex_hex = url.split("ex=")[1].split("&")[0]
                    ...
                    if ex_time < time.time():
                        ...
                        for i, message in enumerate(self.messages):
                            ...
                            if msg_index is not None:
                                break
                        if msg_index is not None:
                            new_msg = self.discord.get_message(
                                self.active_channel["channel_id"],
                                self.messages[msg_index]["id"],
                            )
                            if new_msg:
                                prepared = prepare_message(new_msg)
                                self.messages[msg_index] = prepared
                                for embed in prepared["embeds"]:
                                    if embed["url"].split("?")[0] == url.split("?")[0]:
                                        return embed["url"]
                except Exception:
                    pass
        return url
    ...
    def add_pending_message(
        self, content, nonce, reply_id=None, attachments=None, stickers=None
    ):
        ...
        if stickers:
            stickers = [
                {"name": "sticker", "id": s, "format_type": -1} for s in stickers
            ]
        else:
            stickers = []
    ...
    def upload_native_dialog(self):
        ...
        for file in files:
            self.upload_threads.append(
                threading.Thread(target=self.upload, daemon=True, args=(file,))
            )
            self.upload_threads[-1].start()
        return True
        ...
        try:
            if upload_data:
                ...
                if uploaded is None:
                    ...
                    return
                if uploaded:
                    self.ready_attachments[channel_id][at_index]["upload_url"] = (
                        upload_data["upload_url"]
                    )
                    self.ready_attachments[channel_id][at_index]["upload_filename"] = (
    ...
    def request_missing_members(self, current_guild, messages):
        ...
        if missing_members:
            ...
            self.gateway.request_members(
                current_guild, missing_members, nonce=self.missing_members_nonce
            )
    ...
    def get_chat_chunk(self, past=True, scroll=False):
        ...
        if past:  # when scrolling UP
            ...
            if (
                new_chunk
                and self.get_chat_last_message_id() == self.last_message_id
            ...
            selected_msg, remainder = self.lines_to_msg_with_remainder(
                self.tui.get_chat_selected()[0], space=True
            )
                    ...
                    if selected_id and selected_id > self.messages[0]["id"]:
                        ...
                        for num, message in enumerate(self.messages):
                            if message["id"] == selected_id:
                                selected_msg_index = (
                                    self.msg_to_lines(num, smart=True) - remainder
                                )
                                break
                        else:
    ...
    def preload_chat(self):
        ...
        if self.state and self.state["last_channel_id"]:
            messages = self.discord.get_messages(
                self.state["last_channel_id"], self.msg_num
            )
            if messages is None:  # network error
                return
            # emoji safe
    ...
    def view_log(self):
        ...
        self.messages = []
        log = log_queue.read_log_file(
            os.path.expanduser(
                os.path.join(peripherals.log_path, peripherals.APP_NAME) + ".log"
            )
        )
    ...
    def log_watcher(self, log=[]):
        ...
        self.log_queue_manager = log_queue.LogQueueManager(
            max_size=self.limit_chat_buffer * 2
        )
    ...
    def view_emoji(self, emoji_id):
        ...
        if emoji_path:
            self.media_thread = threading.Thread(
                target=self.open_media, daemon=True, args=(emoji_path,)
            )
            self.media_thread.start()
    ...
    def do_search(self, text):
        ...
        total_search_messages, self.search_messages = self.discord.search(
            self.active_channel["channel_id"]
            if is_dm
            else self.active_channel["guild_id"],
            channel=is_dm,
            content=content,
            channel_id=channel_id,
            author_id=author_id,
        ...
        if assist_type == 1:  # channels
            ...
            if assist_word.lower().startswith("set "):
                ...
                if msg_index is not None:
                    message = self.messages[msg_index]
                    if (
                        "component_info" in message
                        and message["component_info"]["buttons"]
                    ):
                ...
                assist_word.lower().startswith("game_detection_blacklist ")
                and self.enable_game_detection
                and self.game_detection.run
            ):
    ...
    def insert_assist(self, input_text, index, start, end):
        ...
        if self.assist_type == 1:  # channel
            ...
            if self.assist_found[index][1]:
                if input_text.endswith(" ") and input_text not in (
                    "set ",
                    "string_select ",
                    "set_notifications ",
                    "game_detection_blacklist ",
                    "switch_tab ",
                ):
                    self.tui.instant_assist = False
                    command_type, command_args = parser.command_string(input_text)
                    ...
                    return (
                        "",
                        1000000,
                    )  # means its command execution and should restore text from store
    ...
    def open_media(self, path):
        ...
        if (
            ...
            and not uses_pgcurses
        ):
            if not self.terminal_media:
                from endcord import media

                if support_media:
                    self.terminal_media = media.TerminalMedia(
                        self.config, self.keybindings
                    )
            self.update_extra_line()
            self.tui.pause_curses()
    ...
    def update_chat(
        ...
        if self.gateway.legacy:
            for message in self.messages:
                if (
                    ...
                    and not message["referenced_message"]["id"]
                ):
        ...
        if channel:
            ...
            if last_acked_unreads_line and (
                not last_message_id
                or int(last_acked_unreads_line) < int(last_message_id)
            ):
        ...
        if keep_selected:
            ...
            self.tui.set_selected(
                selected_line_new,
                change_amount=change_amount_lines,
                scroll=scroll,
                draw=False,
            )
            ...
            selected_line_new = (
                abs(self.msg_to_lines(select_message_index, full=select_unread))
                + 1
                + full_message
            )
    ...
    def update_tree(self, collapsed=None):
        ...
        if uses_pgcurses:
            if not self.tui.is_window_open and not bool(
                self.tui.get_chat_selected()[1]
            ):
        ...
        while i < 5:
            ...
            if line_map and line_map[0] is not None:
                ...
                while remainder < len(self.chat_map) - (line_index + remainder + 1):
                    ...
                    if not remainder_line_map or (
                        remainder_line_map[0] is None
                        or remainder_line_map[0] != line_map[0]
                        or remainder_line_map[2]
                    ):
                        break
                    remainder += 1
    ...
    def msg_to_lines(self, msg_index, smart=False, full=False):
        ...
        for line_index, line_map in enumerate(self.chat_map):
            if (
                line_map and line_map[0] == msg_index and line_map[4]
            ):  # message root has timestamp range
                if full:
                    next_line_map = self.chat_map[line_index + 1]
                    if (
                        next_line_map
                        and next_line_map[0] == line_map[0]
                        and next_line_map[2]
                    ):
    ...
    def set_mix_seen(self, target_id):
        ...
        if (
            channel
            and channel["last_message_id"]
            and int(channel["last_acked_message_id"]) < int(channel["last_message_id"])
        ):
            self.set_channel_seen(target_id)
            if (
                target_id in self.read_state
                and "last_acked_unreads_line" in self.read_state[target_id]
            ):
            ...
            for guild in self.guilds:
                if guild["guild_id"] == target_id:
                    for channel in guild["channels"]:
                        ...
                        if (
                            channel_r
                            and channel_r["last_message_id"]
                            and int(channel_r["last_acked_message_id"])
                            < int(channel_r["last_message_id"])
                        ):
                            channels.append(
                                {
                                    "channel_id": channel["id"],
            ...
            if channels:
                ...
                for channel in channels:
                    ...
                    if (
                        channel_id in self.read_state
                        and "last_acked_unreads_line" in self.read_state[channel_id]
                    ):
                        self.read_state[channel_id]["last_acked_unreads_line"] = None
                self.update_tree()

    def set_channel_seen(
        self,
        channel_id,
        message_id=None,
        ack=True,
        ...
        if channel:
            ...
            last_message_id = channel["last_message_id"]
            unseen = not last_message_id or int(channel["last_acked_message_id"]) < int(
                last_message_id
            )
            if unseen and (
                not this_channel
                or (not bool(self.tui.get_chat_selected()[1]) and this_channel)
                or force
            ):
    ...
    def set_channel_unseen(
        ...
        if channel:
            last_message_id = channel["last_message_id"]
            if last_message_id and int(channel["last_acked_message_id"]) >= int(
                last_message_id
            ):
            ...
            if last_acked_message_id != 1 or not channel["last_acked_message_id"]:
                self.read_state[channel_id]["last_acked_message_id"] = (
            ...
            if channel.get("last_acked_unreads_line") is None and set_line:
                ...
                if set_line_now:
                    ...
                    self.read_state[channel_id]["last_acked_unreads_line"] = (
                        self.read_state[channel_id]["last_acked_message_id"]
                    )
        else:
        ...
        if channel_id == self.active_channel["channel_id"] and not bool(
            self.tui.get_chat_selected()[1]
        ):
    ...
    def process_msg_events_active_channel(
        self, new_message, selected_line, latest_chat=True
    ):
            ...
            if my_message:
                ...
                if self.slowmodes and self.slowmodes.get(channel_id):
                    if not self.slowmode_times.get(channel_id):
                        self.slowmode_times[channel_id] = self.slowmodes.get(
                            channel_id, 0
                        )
                    if not self.slowmode_thread or not self.slowmode_thread.is_alive():
                        self.slowmode_thread = threading.Thread(
                            target=self.wait_slowmode, daemon=True, args=()
                        )
    ...
    def process_msg_events_cached_channel(self, new_message, ch_num):
        ...
        if op == "MESSAGE_CREATE":
            ...
            if data.get("user_id") == self.my_id and self.show_pending_messages:
                for num, message in enumerate(self.messages):
                    if "pending" in message and message["id"] == new_message.pop(
                        "nonce", None
                    ):
                        self.messages.pop(num)
                        break
        ...
        if op == "MESSAGE_CREATE":
            if data["user_id"] == self.my_id:
                ...
                message_notifications = (
                    2  # 0 - all messages, 1 - only mentions, 2 - nothing
                )
                for guild in self.guilds:
                ...
                if not muted:
                    ...
                    if (
                        (data["mention_everyone"] and not suppress_everyone)
                        or (
                            bool([i for i in my_roles if i in data["mention_roles"]])
                            and not suppress_roles
                        )
                        or (self.my_id in [x["id"] for x in mentions])
                        or (is_dm and new_message_channel_id in self.dms_vis_id)
                    ):
                        if (
                            not this_channel or self.new_unreads
                        ):  # new_unreads already set in process events for active channel
                            ping = True
    ...
    def process_msg_events_ghost_ping(self, new_message):
        ...
        if new_message["op"] == "MESSAGE_DELETE" and not self.keep_deleted:
            ...
            if channel_id in self.read_state and new_message["d"][
                "id"
            ] in self.read_state.get(channel_id, {}).get("mentions", []):
    ...
    def process_new_user_data(self, new_user_data):
        ...
        if guild_roles_changed:
            ...
            self.all_roles = color.convert_role_colors(
                self.all_roles,
                guild_id,
                role_id,
                default=self.config["color_default"][0],
            )
    ...
    def process_call_gateway_events(self, event):
        ...
        if (
            self.in_call
            and event["channel_id"] != self.in_call["channel_id"]
            and event["op"] != "CALL_DELETE"
        ):
            return

        event = self.execute_extensions_methods(
            "on_call_gateway_event", event, cache=True
        )[0]
        ...
        if event["op"] == "CALL_CREATE" and not (self.in_call or self.joining_call):
            if dm["id"] not in self.incoming_calls:
                ...
                self.update_extra_line(
                    custom_text=self.permanent_extra_line, permanent=True
                )
                if event["ringing"]:
                    custom_ringtone = self.config["custom_ringtone_incoming"]
                    linux_ringtone = peripherals.find_linux_sound(
                        self.config["linux_ringtone_incoming"]
                    )
                    if custom_ringtone and os.path.exists(custom_ringtone):
                        ...
                        logger.warning(
                            f"Specified ringtone paths are invalid: {custom_ringtone}, {linux_ringtone}"
                        )
            ...
            event["op"] == "CALL_DELETE" and event["channel_id"] in self.incoming_calls
        ):
            ...
            event["op"] == "STATE_UPDATE" and event["channel_id"] in self.incoming_calls
        ):
    ...
    def process_call_voice_gateway_events(self, event):
        ...
        event = self.execute_extensions_methods(
            "on_call_voice_gateway_event", event, cache=True
        )[0]
                    ...
                    if dm["id"] == self.in_call["channel_id"]:
                        for recipient in dm["recipients"]:
                            if recipient["id"] == event["user_id"]:
                                ...
                                for num, participant in enumerate(
                                    self.call_participants
                                ):
                                    if participant["user_id"] == event["user_id"]:
    ...
    def start_ringing(self, path, loop_delay=1, loop_max=60):
        ...
        if support_media:
            ...
            self.ringer.play_audio_noui(
                path, loop=True, loop_delay=loop_delay, loop_max=loop_max
            )
        else:
    ...
    def start_call(self, incoming=False, guild_id=None, channel_id=None):
        ...
        self.call_participants = []
        self.update_extra_line(
            custom_text="Connecting to voice server.", permanent=True
        )
        ...
        if not incoming:
            ...
            linux_ringtone = peripherals.find_linux_sound(
                self.config["linux_ringtone_outgoing"]
            )
            if custom_ringtone and os.path.exists(custom_ringtone):
    ...
    def update_presence_from_proto(self):
        ...
        custom_status = None
        if (
            "status" in self.discord_settings
            and "status" in self.discord_settings["status"]
        ):
            ...
            if "customStatus" in self.discord_settings["status"]:
                ...
                custom_status = self.discord_settings["status"]["customStatus"].get(
                    "text"
                )
    ...
    def main(self):
        ...
        if logger.getEffectiveLevel() == logging.DEBUG:
            debug.save_json(
                debug.anonymize_guild_folders(self.guild_folders), "guild_folders.json"
            )
        del found
        ...
        self.all_roles = color.convert_role_colors(
            self.all_roles, default=self.config["color_default"][0]
        )
        ...
        if self.config["remember_state"]:
            self.state = peripherals.load_json(
                f"state_{self.profiles['selected']}.json", self.state
            )
        ...
        for folder in self.guild_folders:
            if (
                folder["id"]
                and folder["id"] != "MISSING"
                and folder["id"] not in self.state["collapsed"]
            ):
        ...
        self.gateway.set_want_member_list(
            self.get_members and self.state.get("member_list", True)
        )
        self.tui.set_fun(self.fun)
        ...
        if self.fun in (3, 4):
            ...
            if self.fun == 3:
                self.update_extra_line(
                    "Snow easter egg can be CPU-heavy. Run 'toggle_snow' command to stop it.",
                    timed=False,
                )
            if self.fun == 4 and last_run != (4, 1):
                self.update_extra_line(
                    "Personalized ADs will be shown here. Click this to opt-out.",
                    timed=False,
                )
        ...
        logger.info(
            f"Main loop started after {round((time.time() - self.init_time) * 1000, 3)} ms"
        )
        del self.init_time
        ...
        while self.run:
            ...
            while self.run:
                ...
                if new_message:
                    new_message = self.execute_extensions_methods(
                        "on_message_event", new_message, cache=True
                    )[0]
                    ...
                    if (
                        this_channel
                        and self.get_chat_last_message_id() == self.last_message_id
                    ):  # if its scrolled far up, this channel bot is cached
                    ...
                    # handle cached channels
                        ...
                        if this_channel:
                            ...
                            self.process_msg_events_active_channel(
                                new_message, selected_line, latest_chat=False
                            )
                    ...
                    if not this_channel or (
                        ...
                        and (
                            self.new_unreads
                            or self.ping_this_channel
                            or self.tui.disable_drawing
                            or self.tui.is_window_open()
                        )
                    ):
                ...
                if changed and self.gateway_state == 1:
                    if self.enable_game_detection:
                        ...
                        self.my_activities = rpc_activities + [
                            d
                            for d in game_activities
                            if d["application_id"] not in rpc_apps_ids
                        ]
                    else:
                        self.my_activities = rpc_activities
                    self.gateway.update_presence(
            ...
            if self.send_my_typing and not self.disable_sending:
                ...
                if (
                    ...
                    slowmode_time = self.discord.send_typing(
                        self.active_channel["channel_id"]
                    )
                    self.typing_sent = int(time.time())
                    # check for slowmode
                    if (
                        slowmode_time
                        and slowmode_time != 1
                        and self.active_channel["channel_id"] not in self.slowmode_times
                    ):
                        self.slowmode_times[self.active_channel["channel_id"]] = (
                            slowmode_time
                        )
                        self.update_extra_line(
                            f"Slowmode is enabled, will be able to send message in {slowmode_time} s"
                        )
                        if (
                            not self.slowmode_thread
                            or not self.slowmode_thread.is_alive()
                        ):
                            self.slowmode_thread = threading.Thread(
                                target=self.wait_slowmode, daemon=True, args=()
                            )
            ...
            if new_chat_dim != self.chat_dim:
                ...
                if (
                    ...
                    for dm in self.dms:
                        if dm["id"] == incoming_call_ch_id:
                            new_permanent_extra_line = (
                                formatter.generate_extra_line_ring(
                                    dm["name"],
                                    self.tui.get_dimensions()[2][1],
                                )
                            )
                ...
                if changed_guilds:
                    ...
                    for guild in new_members:  # select guild
                        if guild[0] == self.active_channel["guild_id"]:
                            ...
                            if channel_id in guild[1]:
                                self.member_list = guild[1][channel_id][1]
                                last_index = guild[1][channel_id][0]
                            elif "everyone" in guild[1]:
                                self.member_list = guild[1]["everyone"][1]
                    ...
                    if self.active_channel[
                        "guild_id"
                    ] in changed_guilds and self.state.get("member_list"):
            ...
            if new_member_roles:
                ...
                if nonce is not True and (
                    nonce == self.missing_members_nonce
                    or nonce == self.active_channel["channel_id"]
                ):
                    self.select_current_member_roles()
                    self.update_chat(scroll=False)
            ...
            if not self.forum and self.messages:
                if (
                    selected_line == 0 or text_index == 0
                ) and self.get_chat_last_message_id() != self.last_message_id:
                    self.get_chat_chunk(
                        past=False, scroll=not (text_index == 0 and selected_line <= 2)
                    )
                elif (
                    selected_line >= len(self.chat) - 1
                    or self.tui.get_chat_scrolled_top()
                ) and not self.chat_end:
                    self.get_chat_chunk(
                        past=True, scroll=self.tui.get_chat_scrolled_top()
                    )
            ...
            if assist_type and not self.uploading:
                if (
                    assist_type == 100
                ):  # esc or (" " in assist_word and assist_type not in (5, 6)):
                    self.stop_assist()
                elif assist_type == 6:  # app commands
